...
import { Bell, Settings, Plus, Search, FileText, ChevronLeft, BarChart3, History } from "lucide-react";
      ...
      <aside className="w-72 bg-[#0E1528] border-r border-[#5671FF]/20 flex flex-col justify-between p-6">
        <div className="flex flex-col gap-8">
          ...
          <nav className="flex flex-col gap-2">
            ...
            <Link to="/admin/news" className="flex items-center gap-3 px-4 py-3 rounded-xl bg-[#5671FF] text-white shadow-lg shadow-[#5671FF]/20">
              <FileText className="w-5 h-5" />
              <p className="text-sm font-semibold">Manage News</p>
            </Link>
            ...
            <Link to="/admin/logs" className="flex items-center gap-3 px-4 py-3 rounded-xl text-slate-400 hover:bg-[#5671FF]/10 hover:text-[#5671FF] transition-all">
      ...
      <main className="flex-1 overflow-y-auto flex flex-col">
        <header className="h-20 border-b border-[#5671FF]/10 bg-[#0E1528]/50 backdrop-blur-md flex items-center justify-between px-10 sticky top-0 z-10">
          <div className="flex items-center gap-6">
            <h2 className="text-xl font-bold tracking-tight">Manage News</h2>
            <div className="relative">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400 w-4 h-4" />
              <input className="bg-white/5 border-none rounded-full py-2 pl-10 pr-4 text-sm w-80 focus:ring-2 focus:ring-[#5671FF]/50 text-white placeholder:text-slate-500" placeholder="Search news..." type="text" />
            </div>
          </div>
        ...
        <div className="p-10 flex items-center justify-center flex-1">
          <div className="text-center">
            ...
            <h3 className="text-2xl font-bold mb-2">Manage News</h3>
            <p className="text-slate-400">News management interface coming soon...</p>
          </div>
        </div>
      </main>
    </div>
